...
def init_db():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, aadhaar TEXT UNIQUE NOT NULL, face_pixels BLOB NOT NULL, qr_path TEXT NOT NULL)"
    )
    conn.commit()
    conn.close()
...
def get_trained_model():
    ...
    faces = []
    ids = []
    for user_id, face_pixels in users:
...
@app.post("/register")
async def register(name: str = Form(...), aadhaar: str = Form(...), image_base64: str = Form(...)):
    global global_recognizer

    if len(aadhaar) != 12 or not aadhaar.isdigit():
        return JSONResponse({"status": "error", "message": "Aadhaar must be exactly 12 digits."})
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO users (name, aadhaar, face_pixels, qr_path) VALUES (?, ?, ?, ?)",
